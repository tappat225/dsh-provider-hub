...
var GatewayAdapter = class _GatewayAdapter extends LlmAdapter {
  ...
  async *streamAnthropic(options, gw, baseURL, headers, apiKey, thinkingLevel) {
    let thinking;
    if (gw.anthropicThinking && thinkingLevel !== void 0) {
      const budget = _GatewayAdapter.ANTHROPIC_THINKING_BUDGET[thinkingLevel];
      if (budget === void 0) {
        yield errorFinish(
          `llm-provider-hub: model "${options.model}" declares effort "${thinkingLevel}", but the anthropicThinking passthrough has no budget mapped for it (mapped levels: ${Object.keys(_GatewayAdapter.ANTHROPIC_THINKING_BUDGET).join(", ")}); use a standard level or disable anthropicThinking`,
          "UNSUPPORTED_REASONING_EFFORT"
        );
        return;
      }
  ...
  async *streamOpenAI(options, gw, baseURL, headers, apiKey, wireEffort) {
...
async function discoverModels(request, gw, resolveApiKey) {
  ...
  if (supplied !== void 0 && supplied !== "") headers.authorization = `Bearer ${supplied}`;
  let response;
  try {
    response = await fetch(url, {
      method: "GET",
...
var ProviderHubRuntime = class extends TypertRemoteService {
  ...
  async saveConfig(index, patch) {
    try {
      ...
      const next = config.gateways.map((g, i) => i === index ? { ...g, ...normalizedPatch } : g);
      await this.setGateways(st, next);
