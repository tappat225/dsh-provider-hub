...
async function probeChatConnection(gw, model, resolveApiKey, signal) {
  ...
  try {
    ...
    throw new LlmError3(`could not reach ${redactUrl(url)}`, "UPSTREAM_ERROR", { cause: error });
  }
  if (!response.ok) {
    let text = await response.text().catch(() => "");
  ...
  for await (const chunk of chunks) {
    if (chunk.type === "text-delta") {
      ...
      if (reason.kind === "error") {
        throw new LlmError3(reason.failure?.message ?? "the model stream ended with an error", "UPSTREAM_ERROR");
      }
      if (reason.kind === "aborted") throw new LlmError3("probe aborted by caller", "ABORTED");
      if (!sawOutput) {
        throw new LlmError3(`${redactUrl(url)} answered, but the response stream carried no model output`, "UPSTREAM_ERROR");
      }
      return {
