...
var GatewayAdapter = class _GatewayAdapter extends LlmAdapter {
   ...
   */
  anthropicThinkingBudgets(gw) {
    const merged = { ..._GatewayAdapter.ANTHROPIC_THINKING_BUDGET };
  ...
  async *streamAnthropic(options, gw, headers, apiKey, thinkingLevel) {
    ...
    if (gw.anthropicThinking && thinkingLevel !== void 0) {
      ...
      if (budget === void 0) {
        yield errorFinish(
          `llm-provider-hub: model "${options.model}" declares effort "${thinkingLevel}", but the anthropicThinking passthrough has no budget mapped for it (mapped levels: ${Object.keys(budgets).join(", ")}); map the level in anthropicThinkingBudgets or disable anthropicThinking`,
          "UNSUPPORTED_REASONING_EFFORT"
        );
        return;
      }
...
var ProviderHubRuntime = class extends TypertRemoteService {
  ...
  async saveConfig(index, patch) {
    try {
      ...
      if (patch.anthropicThinkingBudgets !== void 0 && patch.anthropicThinkingBudgets !== null) {
