...
import { LlmError as LlmError4, assertUsableApiKey as assertUsableApiKey2 } from "@deepseek-ai/dsh-llm";
...
import {
  LlmAdapter,
  LlmError,
  isHarnessError as isHarnessError2,
  resolveRetryPolicy as resolveRetryPolicy2
...
function redactUrlsInText(text) {
  return text.replace(/([a-z][a-z0-9+.-]*:\/\/)([^/\s:@]+):([^/\s@]+)@/gi, "$1***:***@").replace(/([a-z][a-z0-9+.-]*:\/\/)([^/\s:@]+)@/gi, "$1***@");
}
...
import { CONTEXT_WINDOW_EXCEEDED_CODE as CONTEXT_WINDOW_EXCEEDED_CODE2, EMPTY_RESPONSE_CODE as EMPTY_RESPONSE_CODE2, QUOTA_EXCEEDED_CODE as QUOTA_EXCEEDED_CODE2 } from "@deepseek-ai/dsh-llm";
var UPSTREAM_ERROR_CODE = "UPSTREAM_ERROR";
var TRANSPORT_ERROR_CODE = "TRANSPORT";
var TIMEOUT_ERROR_CODE = "TIMEOUT";
var RATE_LIMIT_ERROR_CODE = "RATE_LIMIT";
var SERVER_ERROR_CODE = "SERVER";
var AUTH_ERROR_CODE = "AUTH";
var INVALID_REQUEST_CODE = "INVALID_REQUEST";
var ABORTED_CODE = "ABORTED";
var RETRY_AFTER_CAP_MS = resolveRetryPolicy(void 0, "llm-provider-hub retryAfter cap").maxDelayMs;
function usableRetryAfterMs(ms) {
  if (!Number.isFinite(ms) || ms <= 0 || ms > RETRY_AFTER_CAP_MS) return void 0;
  return ms;
}
function parseRetryAfterMs(value, now = Date.now()) {
  ...
  if (/^\d+$/.test(text)) return usableRetryAfterMs(Number(text) * 1e3);
  const at = Date.parse(text);
  if (Number.isNaN(at)) return void 0;
  return usableRetryAfterMs(at - now);
}
function classifyHttpStatus(status, detail, retryAfterMs) {
  ...
  if (status === 429) {
    return { code: RATE_LIMIT_ERROR_CODE, status, ...retryAfterMs === void 0 ? {} : { providerRetryAfterMs: retryAfterMs } };
  }
...
function classifyErrorText(detail) {
...
var PROVIDER_ERROR_CODES = new Map(Object.entries({
...
function classifyProviderError(code, message) {
  const normalized = code?.trim().toLowerCase() ?? "";
  const mapped = normalized === "" ? void 0 : PROVIDER_ERROR_CODES.get(normalized);
  if (mapped !== void 0) return { code: mapped };
  const fromText = classifyErrorText([code ?? "", message].filter((part) => part !== "").join(" "));
  return fromText ?? { code: SERVER_ERROR_CODE };
}
function providerCodeText(value) {
  if (typeof value === "string") return value;
  if (typeof value === "number" && Number.isFinite(value)) return String(value);
  return "";
}
function classifyProviderErrorPayload(payload, raw) {
  const nested = payload.error;
  const message = [payload.message, nested?.message].find((part) => typeof part === "string" && part.trim() !== "") ?? "";
  ...
  const classified = classifyProviderError(code === "" ? void 0 : code, detail);
  return {
...
function classifyStreamEnd(sawTerminalEvent, sawContent) {
  if (!sawContent) {
...
function errorFinish(message, code = UPSTREAM_ERROR_CODE, facts = {}) {
  return {
    type: "finish",
    reason: { kind: "error", failure: { code, message, ...facts } }
  };
}
function abortedFinish(message = "aborted") {
  return {
    type: "finish",
    reason: { kind: "aborted", failure: { code: ABORTED_CODE, message } }
  };
}
function providerErrorFinish(raw) {
  let parsed;
  try {
  ...
  if (parsed !== null && typeof parsed === "object") {
    const classified = classifyProviderErrorPayload(parsed, raw);
  ...
  return errorFinish(message, classifyProviderError(void 0, message).code);
}
...
function blockOf(partial) {
  return partial.blockType === "tool-call" ? { type: "tool-call", id: partial.toolId, name: partial.toolName ?? "", arguments: partial.arguments } : partial.blockType === "reasoning" ? { type: "reasoning", text: partial.text } : { type: "text", text: partial.text };
}
...
async function* openaiCompletionsToChunks(response, signal) {
  ...
  const closeBlocks = () => {
    ...
    return closes.sort((a, b) => a.index - b.index);
  };
  try {
    for await (const sse of iterateSse(response, signal)) {
      if (sse.data === "[DONE]") {
...
async function* responsesSseToChunks(response, signal) {
  ...
  if (terminal === "failed") {
    ...
    const annotated = errorCode === void 0 || errorCode === classified.code ? message : `${message} (${errorCode})`;
...
var GatewayAdapter = class _GatewayAdapter extends LlmAdapter {
  ...
  providerRetryPolicy() {
    return resolveRetryPolicy2(void 0, "llm-provider-hub retryPolicy");
  }
  listModels(provider) {
    const gw = this.deps.gatewayFor(provider);
  ...
  async *stream(options) {
    ...
    try {
      ...
      yield isHarnessError2(error) ? errorFinish(error.message, error.code) : errorFinish(error instanceof Error ? error.message : String(error));
      return;
    }
    ...
    if (!response.ok) {
      ...
      return {
        ...
        chunk: errorFinish(`upstream ${response.status}: ${detail}`, classified.code, {
          ...
          ...classified.providerRetryAfterMs === void 0 ? {} : { providerRetryAfterMs: classified.providerRetryAfterMs }
        })
      };
    }
...
var PROBE_MAX_TOKENS = 64;
var REPLY_SNIPPET_MAX = 120;
var PROBE_MESSAGES = [{ role: "user", content: "hi" }];
function snippet(text) {
  const collapsed = text.replace(/\s+/g, " ").trim();
  return collapsed.length > REPLY_SNIPPET_MAX ? `${collapsed.slice(0, REPLY_SNIPPET_MAX)}\u2026` : collapsed;
}
async function probeChatConnection(gw, model, resolveApiKey, signal) {
  if (gw.api !== "anthropic-messages" && gw.api !== "openai-completions" && gw.api !== "openai-responses") {
    throw new LlmError3(`gateway "${gw.provider}" has an unknown api protocol "${String(gw.api)}"; use "anthropic-messages", "openai-completions" or "openai-responses"`, "UPSTREAM_ERROR");
  }
  const path = gw.api === "anthropic-messages" ? "/messages" : gw.api === "openai-responses" ? "/responses" : "/chat/completions";
  const resolved = resolveEndpointUrl(gw, path);
  if (!resolved.ok) {
    throw new LlmError3(`gateway "${gw.provider}": ${resolved.error}`, "UPSTREAM_ERROR");
  }
  const url = resolved.url;
  let supplied;
  try {
  ...
  try {
    ...
    if (signal?.aborted) throw new LlmError3("probe aborted by caller", "ABORTED", { cause: error });
    const classified = classifyTransportError(error);
    throw new LlmError3(`could not reach ${redactUrl(url)}: ${classified.message}`, classified.code, { cause: error });
  }
  if (!response.ok) {
    let text = await response.text().catch(() => "");
    if (supplied !== void 0 && supplied !== "" && text.includes(supplied)) {
  ...
  const chunks = gw.api === "anthropic-messages" ? anthropicSseToChunks(response, signal) : gw.api === "openai-responses" ? responsesSseToChunks(response, signal) : openaiCompletionsToChunks(response, signal);
  let reply = "";
  let sawOutput = false;
  let usage;
  for await (const chunk of chunks) {
    if (chunk.type === "text-delta") {
      ...
      if (reason.kind === "error") {
        throw new LlmError3(reason.failure?.message ?? "the model stream ended with an error", reason.failure?.code ?? UPSTREAM_ERROR_CODE);
      }
      if (reason.kind === "aborted") throw new LlmError3("probe aborted by caller", "ABORTED");
      if (!sawOutput) {
        throw new LlmError3(`${redactUrl(url)} answered, but the response stream carried no model output`, EMPTY_RESPONSE_CODE2);
      }
      return {
  ...
  throw new LlmError3(`${redactUrl(url)} closed the response without a terminal event`, "UPSTREAM_ERROR");
}
...
function mergeDraftGateway(saved, draft) {
  ...
  for (const [name2, value] of Object.entries(headers)) {
    if (typeof value !== "string") return { error: `extraHeaders.${name2} must be a string` };
    if (/[\r\n]/.test(name2) || /[\r\n]/.test(value)) return { error: `extraHeaders.${name2} must not contain line breaks` };
  }
  const gateway = {
    ...saved,
...
var ProviderHubRuntime = class extends TypertRemoteService {
  ...
  async saveConfig(index, patch) {
    try {
      ...
      if (normalizedPatch.displayName !== void 0) {
        ...
        const providerId = typeof normalizedPatch.provider === "string" && normalizedPatch.provider !== "" ? normalizedPatch.provider : gw.provider;
   ...
   */
  async saveModels(index, models, params) {
    try {
      const st = this.settings();
      if (st === void 0) return fail("settings service unavailable");
      if (st.writable === false) return fail("settings are read-only");
      ...
      for (const row of rows) {
        ...
        const prevSource = MODEL_CATALOG[row.id] !== void 0 ? (gw.modelOverrides ?? {})[row.id] : (gw.customModels ?? []).find((c) => c.id === row.id);
        const unmanaged = {};
        ...
        const built = { ...unmanaged, ...group };
        if (row.name !== "") built.name = row.name;
        if (MODEL_CATALOG[row.id] !== void 0) {
  ...
  async testConnection(index, draft) {
    try {
      ...
      let listingLatency = 0;
      {
        ...
        const timeout2 = setTimeout(() => controller2.abort(), 15e3);
        const started = Date.now();
        try {
          listing = await discoverModels({ signal: controller2.signal }, testGateway, () => this.deps.resolveApiKey(testGateway));
          listingLatency = Date.now() - started;
        } catch (error) {
          modelsError = controller2.signal.aborted ? "the models listing timed out after 15 seconds" : error instanceof Error ? error.message : String(error);
        } finally {
          clearTimeout(timeout2);
        }
      }
      if (modelsError === void 0 && listing !== void 0) {
      ...
      try {
        ...
        const chatError = controller.signal.aborted ? "the chat probe timed out after 15 seconds" : error instanceof Error ? error.message : String(error);
...
function apply(ctx, config) {
  ...
  ctx.llm.registerModelDiscovery(NS, (request) => {
    ...
    if (gw === void 0) {
      throw new LlmError4("llm-provider-hub: model discovery needs a gateway with a baseURL; add one in the plugin settings", "DISCOVERY_FAILED");
    }
