...
function ProviderHubPage(props) {
      ...
      try {
        ...
        if (!r.ok) setStatus({ kind: "err", text: String(r.error ?? "") });
          ...
          if (gateway !== null && typeof gateway === "object" && !Array.isArray(gateway)) {
            ...
            setOverridesText((text) => ({ ...text, [index]: JSON.stringify(entry.gateway.modelOverrides ?? {}, null, 2) }));
            setHeadersText((text) => ({ ...text, [index]: JSON.stringify(entry.gateway.extraHeaders ?? {}, null, 2) }));
          } else {
  ...
  const submitModelForm = () => {
    ...
    if (editing !== null) {
      void (async () => {
        ...
        const r = await call("upsert-model", { index: selected2, entry: model, overwrite: sameId, clearFields: [] });
        if (!r.ok) setStatus({ kind: "err", text: String(r.error ?? "") });
        else {
    ...
    void (async () => {
      ...
      const r = await call("upsert-model", { index: selected2, entry: model, overwrite: false, clearFields: [] });
      if (!r.ok) setStatus({ kind: "err", text: String(r.error ?? "") });
      else {
  ...
  const removeModel = (id) => {
    ...
    if (selected2 === null) return;
    void (async () => {
      const r = await call("delete-model", { index: selected2, id });
      if (!r.ok) setStatus({ kind: "err", text: String(r.error ?? "") });
      else {
  ...
  return import_react.default.createElement(
    ...
    selected === null || selectedEntry === void 0 ? null : import_react.default.createElement(
      ...
      import_react.default.createElement(
        ...
        String(cfg.endpointMode ?? "auto") === "custom" ? fieldRow("endpoint", `${t("endpoint")} *`, t("endpointHint"), t("endpointPlaceholder")) : null,
        Row({
          title: t("api"),
