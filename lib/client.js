...
function TestBanner(props) {
  ...
  return import_react.default.createElement(
    ...
    import_react.default.createElement("span", null, test.ok ? `✓ ${t("testOk")} · ${String(test.latencyMs ?? 0)}ms · ${String(test.modelCount ?? 0)} ${t("testModels")}${models.length > 0 ? ` · ${t("testSeeded")}` : ""}` : `✕ ${t("testFailed")}`),
    import_react.default.createElement(
      "span",
      { className: "phub-test-detail" },
      test.ok ? `GET ${String(test.endpoint ?? "")}${models.length > 0 ? ` → ${models.slice(0, 3).map((m) => m.id).join(", ")}${models.length > 3 ? " …" : ""}` : ""}` : String(test.error ?? "")
    )
  );
}
